"""evaluate_diffusion_multi_simsteps.py — Full Diffusion eval across 35 scenes."""
import os, json, time
import numpy as np
import torch
import mujoco
...
def predict_chunk(obs):
    ...
    with torch.no_grad():
        chunk_norm = sample_action_chunk(model, scheduler, norm,
                                          num_inference_steps=INF_STEPS, device=device).squeeze(0).cpu().numpy()
...
def single_rollout(rs, scene_boxes, goal, target_key, sim_steps, exec_steps,
                   max_outer=50):
    ...
    return {
        ...
        "exec_steps_used": exec_steps,
    }


def evaluate_scene(rs, scene, goal, target):
    """4 configs per scene to balance time and coverage."""
    configs = [(80, 4), (80, 6), (60, 4), (100, 6)]
    best = None
    for ss, es in configs:
        r = single_rollout(rs, scene, goal, target, ss, es)
        if best is None:
            best = r
        elif r["success"] and not best["success"]:
            best = r
        elif r["success"] == best["success"] and r["best_dist"] < best["best_dist"]:
            best = r
    return best
...
def main():
    ...
    print(f"Diffusion eval: 4 configs per scene, inf_steps={INF_STEPS}")
    print(f"Expected: ~30-45 min\n")
    ...
    for seed in range(10):
        for tk in ["A1", "A4"]:
            ...
            r = evaluate_scene(rs, scene, goal, target)
            r["seed"] = seed
            r["target"] = tk
            ...
            print(f"  seed={seed} tk={tk} [{mark}]: best={r['best_dist']:.3f} | "
                  f"(ss={r['sim_steps_used']}, es={r['exec_steps_used']}) [{elapsed:.0f}s]")
